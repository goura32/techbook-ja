...
| 量子化レベル | 適用場面 | 留意点 |
|------||------|
| Q8_0 (8bit) | 最高品質が必要な場面 | メモリ使用料最大 |
| Q6_K (6bit) | 品質とバランス重視 | 8bitに比べて約10%低下 |
| Q5_K_M (5bit) | 標準的な選択 | 品質とサイズのバランス最佳 |
| Q4_K_M (4.5bit) | デフォルト推奨 | 品質低下ほぼなし、推奨 |
| Q3_K_M (3.5bit) | VRAMが制限され場合 | 品質に少し影響 |
| Q2_K (2bit) | メモリが極めて足りない場合 | 品質に大きく影響 |

### モデルの選択フロー

```
1. タスクは？ → チャット/コード生成/要約/翻訳
   ↓
2. 環境は？ → GPUあり/なし、メモリ容量は？
   ↓
3. パラメータ数を選ぶ → 3B(最小)/7B(標準)/13B(中)/70B(大)
   ↓
4. 量子化する → Q4_K_M(推奨)
   ↓
